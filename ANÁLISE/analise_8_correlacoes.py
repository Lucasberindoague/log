...
df_valid = df[df['vlr_nota_avaliacao_cliente_atendimento'].between(1, 5)].copy()
...
variaveis_numericas = ['tempo_resolucao_dias', 'vlr_nota_avaliacao_cliente_atendimento']
...
plt.scatter(df_valid['tempo_resolucao_dias'], df_valid['vlr_nota_avaliacao_cliente_atendimento'], alpha=0.5)
...
estado_stats = df_valid.groupby('cod_uf').agg({
    'tempo_resolucao_dias': ['mean', 'std'],
    'vlr_nota_avaliacao_cliente_atendimento': ['mean', 'std'],
    'des_tipo_servico': 'count'
}).round(2)

# Boxplot de Tempo de Resolução por Estado
plt.figure(figsize=(15, 6))
sns.boxplot(x='cod_uf', y='tempo_resolucao_dias', data=df_valid)
...
servico_stats = df_valid.groupby('des_tipo_servico').agg({
    'tempo_resolucao_dias': ['mean', 'std'],
    'vlr_nota_avaliacao_cliente_atendimento': ['mean', 'std'],
    'cod_uf': 'count'
}).round(2)
...
top_10_servicos = df_valid['des_tipo_servico'].value_counts().head(10).index

# Boxplot de Avaliação por Tipo de Serviço (Top 10)
plt.figure(figsize=(15, 6))
sns.boxplot(x='des_tipo_servico', y='vlr_nota_avaliacao_cliente_atendimento', 
            data=df_valid[df_valid['des_tipo_servico'].isin(top_10_servicos)])
...
df_valid['mes_ano'] = df_valid['dat_criacao'].dt.to_period('M')
temporal_stats = df_valid.groupby('mes_ano').agg({
    'tempo_resolucao_dias': 'mean',
    'vlr_nota_avaliacao_cliente_atendimento': 'mean'
}).round(2)

plt.figure(figsize=(15, 6))
temporal_stats.plot(y=['tempo_resolucao_dias', 'vlr_nota_avaliacao_cliente_atendimento'], marker='o')
...
X = df_valid[['tempo_resolucao_dias', 'vlr_nota_avaliacao_cliente_atendimento']].copy()
...
scatter = plt.scatter(X['tempo_resolucao_dias'], 
                     X['vlr_nota_avaliacao_cliente_atendimento'], 
                     c=X['cluster'], 
                     cmap='viridis', 
                     alpha=0.5)
...
""".format(
    ...
    estado_stats.index[estado_stats[('vlr_nota_avaliacao_cliente_atendimento', 'std')].argmin()],
    estado_stats[('vlr_nota_avaliacao_cliente_atendimento', 'std')].min(),
    "Positiva" if estado_stats[('tempo_resolucao_dias', 'mean')].corr(estado_stats[('des_tipo_servico', 'count')]) > 0 else "Negativa",
    servico_stats.index[(servico_stats[('tempo_resolucao_dias', 'mean')] / servico_stats[('vlr_nota_avaliacao_cliente_atendimento', 'mean')]).argmin()],
    servico_stats.index[(servico_stats[('tempo_resolucao_dias', 'mean')] / servico_stats[('vlr_nota_avaliacao_cliente_atendimento', 'mean')]).argmax()],
    (servico_stats[('vlr_nota_avaliacao_cliente_atendimento', 'std')] < 1).mean() * 100,
    ...
    "Crescente" if temporal_stats['vlr_nota_avaliacao_cliente_atendimento'].corr(pd.Series(range(len(temporal_stats)))) > 0.3 else "Decrescente" if temporal_stats['vlr_nota_avaliacao_cliente_atendimento'].corr(pd.Series(range(len(temporal_stats)))) < -0.3 else "Estável",
